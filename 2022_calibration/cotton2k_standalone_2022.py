...
# ─────────────────────────────────────────────────────────────────────
GDD_SQUARING     = 345.0   # 2022标定年: 沿用品种基准值(经2022实测GDD序列核验合理)
GDD_FLOWERING    = 630.0   # 2022标定年: 沿用品种基准值(经2022实测GDD序列核验合理)
GDD_BOLL_SET     = 810   # 坐铃
GDD_BOLL_OPENING = 1479.0   # 2022实测: DAE=126, 9月4日吐絮始
GDD_DEFOLIATION  = 1550   # 2022标定年: 沿用品种基准值(非自由参数)
GDD_MATURITY     = 1620   # 2022标定年: 沿用品种基准值(经2022实测物候窗口核验)

# 形态参数 (塔河2号 2022 阿拉尔实测)
# 2022标定年: MAX_HEIGHT 由本年15个实测锚点(出苗后GDD vs 株高)直接逻辑斯谛拟合,
# 与碳供给路径无关(高度仅为GDD的函数), 故可放心做年度专属标定。
# scipy.curve_fit 独立复现: Hmax=81.928, k=0.00395, mid_GDD=703.69, RMSE=2.77cm, NSE=0.989
# ─────────────────────────────────────────────────────────────────────
MAX_HEIGHT  = 81.93  # cm — 2022标定: 株高逻辑斯谛拟合上限 (15点直接拟合, 与碳路径无关)
...
_PARTITION_ANCHORS = [
    ...
    # 2022标定年 v1: 2022辐射较2025低12.7%(18.5 vs 21.2 MJ/m2/d), 在Beer-Lambert
    # 一维消光约束下, 同样的分配比例不足以支撑实测LAI, 故在DAS35-100窗口
    # 上调叶分配、下调铃分配 (根/茎大致不变), 以2022年15个实测LAI锚点逐轮校验.
    (10,  0.52, 0.28, 0.00, 0.15),   # 苗期 (出苗早期碳路径耦合尚弱, 保持不变)
    (35,  0.42, 0.30, 0.00, 0.13),   # 初蕾DAS=35(DAE=35)
    (48,  0.37, 0.30, 0.04, 0.14),   # 盛蕾末DAS=48
    (63,  0.27, 0.21, 0.36, 0.13),   # 初花DAS=63(DAE=53): leaf 0.22→0.27, boll 0.38→0.36
    (70,  0.19, 0.19, 0.49, 0.13),   # 盛花DAS=70: leaf 0.16→0.19, boll 0.51→0.49 (注:此前已是2022值)
    (73,  0.14, 0.17, 0.57, 0.12),   # 打顶日DAS=73(DAE=73): leaf 0.10→0.14, boll 0.60→0.57
    (83,  0.09, 0.12, 0.69, 0.10),   # 打顶后DAS=83: leaf 0.06→0.09, boll 0.71→0.69
    (100, 0.05, 0.06, 0.81, 0.08),   # 盛铃峰DAS=100: leaf 0.03→0.05, boll 0.82→0.81
    (119, 0.00, 0.04, 0.90, 0.06),   # 盛铃后 (临近吐絮, 叶分配已趋零, 保持不变)
    (136, 0.00, 0.02, 0.93, 0.05),   # 吐絮始 (不变)
    (170, 0.00, 0.02, 0.93, 0.05),   # 吐絮末 (不变)
]
...
TOPPING_LEAF_LOSS_FRAC_STD = 0.06   # 2022标定: 实测打顶后LAI仍由3.969升至4.063(DAE73→80),
                                     # 表明打顶瞬时损失很小(主要效应是阻断顶端新叶供给,
                                     # 已由分配锚点DAS83+趋零体现), 故下调0.18→0.06
...
def run_simulation(weather_path: str) -> pd.DataFrame:
    ...
    for i in range(DAYS):
        ...
        height = MAX_HEIGHT / (1 + np.exp(-0.00395 * (gdd_ae - 703.7)))   # 2022标定: 15点直接拟合
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("单独cotton2k  —  新疆阿拉尔 塔河2号 (2022标定年)")

...
密度:    20 万株/hm² = 20 株/m²
播种:    2022-04-21  出苗: 2022-05-01  打顶: 2022-07-13 (播后83天, DAE=73) [2022标定年]
"""
...
from cotton2k_daily import Cotton2KDaily, GCH2O_TO_GDM, GINNING_RATIO, \
...
BOLL_COUNT_TARGET  = 8.2   # 2022标定年实测综合均值
...
# 实现说明 (方案A v2, 修正三个缺陷):
#   v1 缺陷1: 用 W_lf×SLA (raw) 而非 W_lf×SLA×f_vis (visible), 苗期 raw/visible≈1.54
#             → 苗期误判 C2K>>GroIMP → 错误下压 W_lf -35 g/m²
#   v1 缺陷2: 在 step_day() 前执行, 用上一天 GroIMP LAI, 蕾花期 raw≈gro_prev 导致漏检
#             → 33天大偏差(25-36%)全部漏检
#   v1 缺陷3: W_lf 下限设为 3.5 g/m², 干扰衰老期自然下降
#   v2 修正: step_day() 后执行, 用当天 visible LAI 对比当天 GroIMP LAI (同口径同步)
#            DAS>35 门槛规避 GroIMP 冷启动期失真, W_lf 下限改为 0.5 g/m²
...
MEASURED_LINT_KGHA  = 2948.0   # 2022标定年实测皮棉 (7020×0.420=2948 kg/ha)
MEASURED_SEED_KGHA  = 7020.0   # 2022实测籽棉
MEASURED_PLANT_HT   = 80.5   # cm (2022实测最大株高)
MEASURED_NODES      = 17.0   # 2022打顶后主茎节数

# 实测 LAI 关键节点
# 更新依据: 有效积温与植株生长关系数据表2.xlsx (2022 阿拉尔, 15个实测节点)
# 峰值LAI=4.20@DAE=80(盛铃初期), 打顶日(DAE=75)LAI=4.00
MEASURED_LAI_PEAK     = 4.063   # 2022标定年峰值LAI (DAE=80, 4区均值)
MEASURED_LAI_PEAK_SD  = 0.100   # 2022实测标准差
MEASURED_LAI_PEAK_DAS = 80   # 出苗后峰值天

# 完整 15 个实测 LAI 锚点 (田间实测, 4区均值, DAE 0-138)
MEASURED_LAI_SERIES = [
    # 2022标定年实测15个LAI锚点 (田间实测, 4区均值)
    (0,   0.013), (9,   0.184), (28,  0.735), (35,  0.966),
    (53,  1.958), (61,  2.939), (66,  3.331), (70,  3.654),
    (73,  3.969), (80,  4.063), (90,  3.985), (99,  3.468),
    (109, 3.050), (119, 2.526), (138, 1.860),
]

# 实测产量 + 单株
MEASURED_OPEN_BOLLS_PER_PLANT    = 8.2   # 2022标定年实测结铃数均值
MEASURED_BOLL_WEIGHT_G           = 6.30   # 2022实测铃重 6.30±0.72g
MEASURED_SEED_COTTON_G_PER_PLANT = 35.1   # 2022实测单株籽棉重
...
# 5. LAI 一致性软约束 (方案A v2)
# =======================================================================
# GroIMP 有效性门槛 (数据驱动标定, 基于2022年实测运行):
#
# GroIMP 3D 几何冷启动滞后期持续至 DAS≈44:
#   DAS36 (Day37): GRO/C2K ratio=0.71  → GroIMP 严重低估 (C2K 更接近实测)
#   DAS40 (Day41): GRO/C2K ratio=0.81  → GroIMP 仍在建成过渡期
#   DAS45 (Day46): GRO/C2K ratio=0.93  → GroIMP 首次连续3天稳定≥0.90 ← 可信起点
#
# 证据: v2(Day37门槛) 在 Day37-41 触发5次错误下压 (-15.41 g/m²),
#   此时 C2K LAI (1.08-1.14) 比 GroIMP (0.81-0.91) 更接近实测 (~1.0-1.1),
#   Nudge 将更准确的 C2K 向不准确的 GroIMP 拉近, 方向错误.
#   修正: 门槛延至 Day46 (DAS=45), 与 GRO/C2K ratio 首次稳定>=0.90 对齐.
NUDGE_GROIMP_START_DAY = 46    # groimp_day >= 此值才允许触发 (对应 DAS=45)
#                               # 数据依据: GRO/C2K ratio 连续3天>=0.90 首次出现于 Day46
...
def apply_wlf_nudge(
    ...
    修正口径 (v2 关键修正):
      C2K LAI  = rec['leaf_area_index'] = W_lf × SLA × f_vis  (visible, 与实测同口径)
      GRO LAI  = groimp_lai_3d                                  (GroIMP 3D, 与实测同口径)
      两者均为当天同步值, 彻底消除 v1 的 raw/visible 错位与时序错位.

    有效性门槛 (基于2022实测数据标定):
      groimp_day >= 46 (DAS=45): GRO/C2K ratio 首次连续3天稳定>=0.90 的起点.
      DAS<45 期间 GroIMP 几何冷启动严重滞后 (ratio=0.15-0.88), 以此修正 C2K 方向错误.

    修正机制:
      目标 W_lf = groimp_lai_today / (sla × f_vis)
      nudge = (target - current) × (1 - exp(-1/tau)) × dvs_factor
      限幅: |nudge| <= |gap| × MAX_RATE × dvs_factor
    ...
    """
    # 门槛1: GroIMP 几何建成期 (DAS<45) 不可信, 直接跳过
    # 数据依据: 2022年实测中 GRO/C2K ratio 在 DAS<45 时为 0.15-0.88,
    # 此期间 C2K visible LAI 更接近田间实测值.
    # DAS45 (Day46) 起 ratio 连续稳定>=0.90, GroIMP 几何可信.
    ...
    # DAS<=80: 叶片建成期, 全力修正
    # DAS 81-110: 线性衰减到 0.10, 避免铃期过度干预
    # DAS>110: 微弱修正, 仅防止极端分歧
    if groimp_day <= 80:
        dvs_factor = 1.0
    elif groimp_day <= 110:
    ...
    # 计算当日 SLA × f_vis (visible SLA), 用于将目标 visible LAI 反算为目标 W_lf
    # das = step_day 执行后 model.current_day 已推进一天, 需回退一天
    ...
    sla_vis = sla * f_vis   # visible SLA: visible_LAI = W_lf × sla_vis
...
def prepare_c2k_data_for_groimp(
    ...
    data: Dict[str, Any] = {
        ...
        # Nudge 消融控制字段 — RGG 端读取后决定是否执行 c2kNudgeLAI() (诊断用)
        # 方案A 下 Nudge 已在 Python 端执行, 此字段仍发送以保持 RGG 端诊断一致性
...
def run_daily_coupling(
    ...
    print('=' * 70)
    print(f'  cotton2k与GroIMP耦合 — 逐日耦合服务器 [2022标定年]')
    ...
    print(f'  播种: 2022-04-21  出苗: 2022-05-01  打顶: 2022-07-13 (播后83天, DAE=73)')
    print('=' * 70)
    ...
    print(f'  实测籽棉: {MEASURED_SEED_KGHA:.0f} kg/ha → 皮棉: {MEASURED_LINT_KGHA:.0f} kg/ha (衣分42%) [2022标定年]')
    print('=' * 70)
    ...
    try:
        ...
        # Nudge 追踪 (方案A: Python 端 W_lf 修正)
        nudge_log: List[Dict[str, Any]] = []
        nudge_total_delta = 0.0   # 累积修正量 (g/m²)
        ...
        for groimp_day in range(1, GROWTH_PERIOD + 1):
            ...
            # 比较当天 C2K visible LAI vs 当天 GroIMP LAI (同步同口径)
            nudge_delta = 0.0
            nudge_fired = False
            if NUDGE_ENABLED and groimp_lai >= 0.0 and c2k_lai >= 0.0:
                nudge_delta, c2k_lai_used, nudge_fired = apply_wlf_nudge(
...
def save_results(model, coupling_log: List[Dict[str, Any]], output_dir: str,
    ...
    # ── E_pre. Nudge 消融诊断 (方案A v2: 同步同口径, step_day 后执行) ──
    print(f'\n  === Nudge 消融实验状态 (方案A v2: visible LAI 同步比对) ===')
    nudge_mode_str = ('NudgeON (v2: visible口径, step后, DAS>=45门槛)'
                      if NUDGE_ENABLED else
                      'NudgeOFF ★ 消融 — LAI一致性软约束已关闭')
    print(f'  运行模式:  {nudge_mode_str}')
    n_nudge_days    = sum(1 for e in coupling_log if e.get('nudge_fired', False))
    total_nudge_wlf = sum(_safe_float(e.get('nudge_wlf_delta', 0)) for e in coupling_log)
    if not NUDGE_ENABLED:
        ...
        pos_days = sum(1 for e in coupling_log
                       if e.get('nudge_fired') and e.get('nudge_wlf_delta', 0) > 0)
        neg_days = sum(1 for e in coupling_log
                       if e.get('nudge_fired') and e.get('nudge_wlf_delta', 0) < 0)
        print(f'  Nudge 有效窗口:     DAS>=45 (groimp_day>={NUDGE_GROIMP_START_DAY}) '
              f'且 GroIMP LAI>={NUDGE_GROIMP_MIN_LAI}')
        print(f'  门槛依据:           GRO/C2K ratio>=0.90 连续3天首次出现于 Day46 (DAS=45)')
        print(f'  Nudge 触发次数:     {n_nudge_days} 天'
              f'  (上推 {pos_days} 天 / 下压 {neg_days} 天)')
        print(f'  累积 W_lf 修正量:   {total_nudge_wlf:+.2f} g/m²')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=f'cotton2k与GroIMP耦合 — 逐日耦合服务器 [2022标定年] '
                    f'(配合 CottonModel_2022_Coupled.rgg)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument('--weather', default='weather.csv', help='气象 CSV')
    ...
    parser.add_argument('--nudge-off', action='store_true', default=False,
                        help='消融实验: 关闭LAI一致性软约束(Nudge); '
                             '每日向RGG发 nudge_enabled=false, '
                             '配合 CottonModel_2022_NudgeOFF.rgg 使用')
    ...
    try:
        ...
        if not nudge_on:
            print('[消融实验] --nudge-off 激活: 配合 CottonModel_2022_NudgeOFF.rgg')

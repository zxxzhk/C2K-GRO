...
# 1. 站点/物候/品种配置 (新疆阿拉尔 2022 对照年, 塔河 2 号)
# =====================================================================
...
GDD_DEFOLIATION  = 1550   # T1统一(2025基准): 脱叶期(品种属性, 年型延后应由低温驱动)
GDD_MATURITY     = 1620   # T1统一(2025基准): 成熟(品种属性; 注: 同时是衰老/DVS/Kc的分母)
...
# ─────────────────────────────────────────────────────────────────────
MAX_HEIGHT = 84.62   # cm; T1统一(2025基准): 逻辑斯谛拟合值(品种属性)
...
RUE              = 3.4
PAR_FRACTION     = 0.48
MAINT_RESP_COEFF = 0.005   # T1统一(2025基准): 棉花活组织文献下限 (Reddy 1996)
INERTIA_COEF     = 0.00062  # T1统一(2025标定值): 产量惯性系数; 2025上由3163kg/ha标定(第二轮), 验证年冻结沿用
                              # 依据: 2022实测产量2948 kg/ha (468kg/亩×15×0.42)
                              # 文献物理范围: 0.002-0.010 g/(g·d) (Amthor 2000)
...
# 灌溉日期 (2022对照年: 按播后天数DAP复用2025方案, 整体-2天对齐04-21播种)
# ─────────────────────────────────────────────────────────────────────
# 灌溉制度：日期 → 灌水量(mm)
...
# ─────────────────────────────────────────────────────────────────────
_SLA_ANCHORS = [   # T1统一(2025基准): SLA-DAE曲线为品种属性, 全块采用2025标定值
    (5,   0.0220, '苗期'),
    (11,  0.0220, '苗期'),
    (21,  0.0210, '苗末'),
    (28,  0.0200, '初蕾'),
...
def get_dynamic_sla(das: float, ks: float = 1.0, kn: float = 1.0) -> float:
    ...
    # 花铃-盛铃期 SLA 下限守卫 (DAS 55-75) — T1统一(2025基准)
...
# ─────────────────────────────────────────────────────────────────────
_PARTITION_ANCHORS_COUPLED = [   # T1统一: 比例值=2025品种规则; DAS位置=2022实际物候(T3)
    # (DAS_中点, alphaLeaf, alphaStem, alphaBoll, alphaRoot)
    (10,  0.52, 0.28, 0.00, 0.15),   # 苗期
    (35,  0.45, 0.30, 0.00, 0.15),   # 初蕾
    (48,  0.38, 0.32, 0.05, 0.15),   # 盛蕾
    (63,  0.28, 0.25, 0.30, 0.12),   # 初花(2025值@2022 DAS63)
    (70,  0.18, 0.20, 0.50, 0.12),   # 盛花
    (73,  0.10, 0.18, 0.62, 0.10),   # 打顶日(2025值@2022 DAS73)
...
_PARTITION_ANCHORS = [   # T1统一: 比例值=2025品种规则; DAS位置=2022实际物候(T3)
    # (DAS_中点, alphaLeaf, alphaStem, alphaBoll, alphaRoot)
    (10,  0.52, 0.28, 0.00, 0.15),   # 苗期
...
def leaf_senescence(gdd_ae, tmin, lai, date=None, rad_buffer=1.0):
    ...
    if date is not None:
        ...
        if 73 <= das_local <= 80:
            r += 0.004 * rad_buffer   # T1统一(2025基准): 打顶后新叶补偿
        elif 81 <= das_local <= 95:
            r += 0.008 * rad_buffer   # T1统一(2025基准): 盛铃初缓衰老
        elif 96 <= das_local <= 110:
            r += 0.015 * rad_buffer   # T1统一(2025基准): 盛铃后中速衰老
        elif 111 <= das_local <= 145:
            r += 0.012 * rad_buffer   # T1统一(2025基准): 初絮持续衰老
    if date is not None:
...
TOPPING_LEAF_LOSS_FRAC = 0.18   # T1统一(2025基准): 顶端打顶损失约18%叶质量
...
def soil_moisture_bucket(state: Dict[str, Any], date: datetime, gdd_ae: float,
    ...
    LAYER_DEPTH = 300.0   # mm — T1统一(300mm根区深): 膜下滴灌主根区0-30cm; 两年物理模型一致
...
class Cotton2KDaily:
    ...
    def step_day(self, coupling_inputs: Optional[Dict[str, float]] = None) -> Optional[Dict[str, Any]]:
        ...
        dSq = net * c['square']
        dBl = net * c['boll']
        # T1统一(2025基准): DVS≥1.5 充实中后期铃封顶 cap=0.70×net (Pettigrew 2004 库容限制)
        ...
        # ── 6) 形态 ──────────────────────────────────────
        # 株高 logistic (T1统一2025基准, 出苗后GDD): k=0.00422, mid=453.9
        self.height = MAX_HEIGHT / (1 + np.exp(-0.00422 * (gdd_ae - 453.9)))   # T1统一(2025基准)
        ...
        # inertia_coef 精确重标定 (结构系数, 两年统一以保证验证严谨性)
        # 根因: coef=0.010 导致产量3780(+19.5%)，收获效率93.3%(超文献上限80%)
        # 标定方法: 结构系数 0.0007 由 2025 实验组反推 (2025目标皮棉3163/衣分0.42=7531 kg/ha);
        #   2022 对照组沿用同一结构系数, 不单独拟合 → 2022 产量为模型在统一参数下的"预测", 而非拟合.
        #   2022 自身田间实测皮棉基准 = 2948 kg/ha (见 daily_coupling_server_2022.MEASURED_LINT_KGHA),
        #   验证误差由耦合服务器以 2948 为准计算 (路径1 ≈ 3074 kg/ha, +4.3%).
        # [2025推导明细] dBl_sum(DAE60-142)=7194; inertia_per_unit=480324(coef=1时);
        #   coef=(7531-7194)/480324=0.000702≈0.0007; 校验 (7194+0.0007×480324)×0.42=3163 ✓
        #
        # ── 产量累积 (T1统一: 与2025标定结构完全一致) ────────────────────
        # 移除原"DVS≥2.0 / α_boll=0.30"分支 —— 0.30由2022实测2948 kg/ha反向标定,
        # 属数据泄漏, 破坏"2022为2025标定模型之独立验证"前提。
        # 改用2025标定结构: 铃封顶已上移至分配段(DVS≥1.5, cap=0.70); 惯性系数恒为INERTIA_COEF(=0.00062)。
...
if __name__ == '__main__':
    ...
    print('=' * 72)
    print(f' 单独cotton2k — 新疆阿拉尔塔河2号 (2022对照年)')
    print('=' * 72)
    print(f'  weather: {wf}')
    print()
    print(f'  ── 栽培制式参数 ──')
    ...
    print(f'  MAX_HEIGHT = {MAX_HEIGHT} cm  (逻辑斯谛拟合, RMSE=3.36cm)')
    print(f'  逻辑斯谛参数: k=0.00422, mid_GDD=453.9 (T1统一2025基准, 出苗后GDD)')
    print()

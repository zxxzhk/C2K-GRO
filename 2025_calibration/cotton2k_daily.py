...
# 1. 站点/物候/品种配置 (新疆阿拉尔 2025, 塔河 2 号)
# =====================================================================
...
# MAX=84.62, k=0.00422, mid_GDD=453.9, RMSE=3.36cm
# MAX_NODES: 打顶后主茎节数约15-17节
# ─────────────────────────────────────────────────────────────────────
MAX_HEIGHT = 84.62   # cm; 逻辑斯谛拟合值 (实测稳定值83-86cm)
...
# 标定目标: 2025 耦合实验皮棉产量 ≈ 3162.6 kg/ha (实测)
# 当前值状态:
#   ★ 0.00062 为 100mm 桶深下标定结果 (耦合 2025, 旧)
#   ★ 300mm 桶深下 Ks 更高(更少虚假胁迫), 净光合偏高, 产量基线上移
#   ★ 预期新值在 0.00052–0.00060 之间; 运行 calibrate_inertia_300mm.py 自动求解
# 修改方法: 不要手动改此处; 使用配套脚本 calibrate_inertia_300mm.py 自动写入
# ─────────────────────────────────────────────────────────────────────
INERTIA_COEF = 0.00062   # 300mm根区深度下的最终标定值；已用calibrate_inertia_300mm.py校验，
                          # 以300mm配置下实际耦合产量为输入，2025年误差+1.06%、2022年误差-0.08%，
                          # 均在2%收敛判据内，判定无需重标定
...
_SLA_ANCHORS = [
    ...
    (59,  0.0185, '盛花'),
    (66,  0.0188, '盛花2'),
    (71,  0.0175, '花铃初'),   # 修正: 0.0190→0.0175，抑制打顶前LAI虚高（实测峰在DAE=80非DAE=71）
    (75,  0.0183, '打顶日★'),   # 修正: 0.0200→0.0183，与W_lf约220 g/m²对应LAI=4.01 ✓
    (80,  0.0193, '盛铃峰★'),   # 修正: 0.0210→0.0193，与W_lf约220 g/m²对应LAI=4.25（允许+1.2%）
    (90,  0.0193, '盛铃'),      # 修正: 0.0210→0.0193，保持与DAE=80平滑衔接
    (100, 0.0183, '盛铃后'),    # 修正: 0.0200→0.0183，与实测LAI=3.60/W_lf对应
    (110, 0.0185, '初絮'),   # 反推: LAI2.80/W_lf152=0.0184, 取0.0185
    (120, 0.0160, '吐絮'),   # 反推: LAI2.10/W_lf131=0.0160, 精确匹配
    (130, 0.0135, '吐絮盛'),
    (150, 0.0110, '吐絮后期'),
    (170, 0.0090, '吐絮末'),
]
...
_PARTITION_ANCHORS_COUPLED = [
    ...
    (35,  0.45, 0.30, 0.00, 0.15),   # 初蕾: 继续增叶
    (48,  0.38, 0.32, 0.05, 0.15),   # 盛蕾: 叶为主，蕾开始分配
    (57,  0.28, 0.25, 0.30, 0.12),   # 初花: 叶铃并重
    (70,  0.18, 0.20, 0.50, 0.12),   # 盛花: 铃为主
    (76,  0.10, 0.18, 0.62, 0.10),   # 打顶日: 大幅降低叶分配
    (84,  0.05, 0.12, 0.73, 0.10),   # 打顶后: 铃强分配
    (100, 0.02, 0.06, 0.84, 0.08),   # 盛铃峰: 极高铃分配
    (119, 0.00, 0.04, 0.90, 0.06),   # 盛铃后: 全部给铃
...
TOPPING_LEAF_LOSS_FRAC = 0.18   # 顶端打顶损失约18%叶质量
...
class Cotton2KDaily:
    ...
    def step_day(self, coupling_inputs: Optional[Dict[str, float]] = None) -> Optional[Dict[str, Any]]:
        ...
        # ── 6) 形态 ──────────────────────────────────────
        # 株高：有效积温数据表2逻辑斯谛拟合(出苗后GDD): MAX=84.62, k=0.00422, mid=453.9
        self.height = MAX_HEIGHT / (1 + np.exp(-0.00422 * (gdd_ae - 453.9)))
...
if __name__ == '__main__':
    ...
    print('=' * 72)
    print(f' 单独cotton2k — 新疆阿拉尔塔河2号 (2025)')
    print('=' * 72)
    print(f'  weather: {wf}')
    print()
    print(f'  ── 栽培制式参数 ──')
    ...
    print(f'  逻辑斯谛参数: k=0.00422, mid_GDD=453.9 (出苗后GDD)')
    print()
